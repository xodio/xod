...
export const getProjectPath = def(
  ...
  R.pipe(XP.getProjectName, fsSafeName, name => `.${path.sep}${name}`)
);
...
const getImplFiles = def(
  ...
  (projectPath, patch) =>
    R.compose(
      R.map(([implType, content]) => ({
        path: filePath(
          projectPath,
          getPatchFolderName(patch),
          getImplFilenameByType(implType)
        ),
        content,
      })),
      R.toPairs,
...
export const arrangeByFiles = def(
  ...
  project => {
    ...
    const patchFiles = R.compose(
      R.chain(
        R.converge(R.unapply(XF.concatAll), [
