...
export const isProjectFile = def(
  ...
  R.pipe(R.prop('path'), path.basename, R.equals('project.xod'))
);

export const isPatchFile = def(
  'isProjectFile :: AnyXodFile -> Boolean',
  R.pipe(R.prop('path'), path.basename, R.equals('patch.xodp'))
);
...
export const getImplFilenameByType = def(
  ...
  type =>
    R.compose(R.head, R.values, R.pickBy((v, k) => R.equals(k, type)))(
      IMPL_FILENAMES
    )
);
...
export const getImplTypeByFilename = def(
  ...
  filename =>
    R.compose(R.head, R.keys, R.pickBy(R.equals(filename)))(IMPL_FILENAMES)
);
