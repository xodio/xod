...
/**
 * Returns a map of pins for patch that exists.
 * So Pins are fully valid, contains proper types and etc.
 */
export const getPinsForExistentPatch = def(
  'getPinsForExistentPatch :: Node -> Patch -> Map PinKey Pin',
...
/**
 * Returns a map of pins for Node, that points to unexistent Patch.
 * These pins are created from links, connected with some Pins in this Node.
 */
export const getPinsForUnexistentPatch = def(
  'getPinsForUnexistentPatch :: Node -> Patch -> Map PinKey Pin',
  (node, currentPatch) => {
    ...
    return R.compose(
      ...
      R.mapObjIndexed(
        (links, direction) => R.compose(
          mapIndexed(
            (pinKey, idx) => Pin.createUnknownPin(pinKey, direction, idx)
          ),
          R.keys,
          R.groupBy(R.ifElse(
 ...
 * add "unknown" pin to the Patch, to connect this link somewhere.
 * E.G.
 ...
 * unknown type, to render link somehow and mark it as dead.
 */
export const ensureLinksConnectedToPins = def(
  'ensureLinksConnectedToPins :: Node -> Patch -> Map PinKey Pin -> Map PinKey Pin',
  (node, currentPatch, pins) => {
    ...
    const rejectLinkConnectedWithExistentPins = R.reject(
      R.either(
    ...
    return R.compose(
      ...
      R.when(
        R.complement(R.isEmpty),
        R.compose(
          R.map(R.apply(Pin.createUnknownPin)),
          R.unnest,
          R.values,
          R.mapObjIndexed(
      ...
      rejectLinkConnectedWithExistentPins,
      listLinksByNode
    )(nodeId, currentPatch);
  }
);
