...
const fetchUploadConfig = boardId =>
  fetch(`${getUploadConfigUrl()}${boardId}`)
    .then((res) => {
...
export const getUploadConfig = boardId => fetchUploadConfig(boardId)
  .catch(retryOrFail(
    RETRY_DELAYS,
...
const getToolPath = R.curry(
  (uploadConfig, localConfig) => path.normalize(
    `${localConfig.toolPath}/${uploadConfig.tool.name}/${uploadConfig.tool.version}/`
  )
);
...
// :: UploadConfig -> Path
export const getToolVersionPath = R.converge(
  path.join.bind(path),
  [
    getToolName,
    getToolVersion,
...
export const runCommand = cmd => cpp.exec(cmd).then(normalizeChildProcessResult);
...
const prepareBoardUpload = async (uploadConfig, localConfig) => {
  ...
  if (!uploadConfig.disableFlushing) { await flushSerialBuffer(portName); }

  const ports = await listPorts();

  if (uploadConfig.touch1200bps) { await touchPort1200(portName); }
  if (!uploadConfig.waitForPort) { return [uploadConfig, localConfig]; }
