...
describe('Utils', () => {
  ...
  it('getToolVersion() returns version', () => {
    const version = Utils.getToolVersion('arduino:samd:arduino_zero_native', 'bossac', packageIndex);
    assert.equal(version, '1.7.0', 'Should return proper version of a tool from packageIndex');
  });
  it('getToolsByFqbn() returns a list of tools', () => {
    const tools = Utils.getToolsByFqbn('arduino:samd:arduino_zero_native', packageIndex);
  ...
  it('getToolVersionDirectory() returns correct path', () => {
    const dir = Utils.getToolVersionDirectory('avrdude', '1.0.0-arduino1', '/xod/arduino/tools/');
    assert.strictEqual(dir, path.normalize('/xod/arduino/tools/avrdude/1.0.0-arduino1'));
  });
