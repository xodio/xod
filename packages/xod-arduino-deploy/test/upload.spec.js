...
describe('Uploader', () => {
  ...
  it('composeCommand() for `arduino:samd:arduino_zero_native` (bossac)', async () => {
    ...
    assert.strictEqual(
      cmd,
      `"${execCmd}" --port=/dev/cu.usbmodem1411 -U true -e -w -b "${
        binaryFile
      }" -R`
    );
  });
  it('composeCommand() for `arduino:samd:mzero_pro_bl_dbg` (openocd-withbootsize)', async () => {
    ...
    const execCmd = path.normalize(
      `${packagesDir}/arduino/tools/openocd/0.9.0-arduino6-static/bin/openocd${
        winExt
      }`
    );
    ...
    const config = path.normalize(
      `${
        packagesDir
      }/arduino/hardware/samd/variants/arduino_mzero/openocd_scripts/arduino_zero.cfg`
    );
    const binaryFile = path.join(artifactsDir, 'test.cpp.bin');
  ...
  it('composeCommand() for `arduino:samd:tian` (avrdude)', async () => {
    ...
    assert.strictEqual(
      ...
      `"${execCmd}" -C "${
        config
      }" -p atmega2560 -c wiring -P /dev/cu.usbmodem1411 -b 57600 "-Uflash:w:${
        binaryFile
      }:i"`
    );
  });
