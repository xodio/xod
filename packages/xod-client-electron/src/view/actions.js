...
export const createAsyncAction = ({
  ...
  if (!silent) {
    ...
    if (processMsg) {
      ipcRenderer.once(`${eventName}:process`, (sender, payload) =>
        processProgressed(
  ...
  ipcRenderer.once(`${eventName}:complete`, (sender, payload) => {
    if (!silent) {
  ...
  ipcRenderer.once(`${eventName}:error`, (sender, err) => {
    // eslint-disable-next-line no-console
    console.error(
      `Unhandled error returned in event '${eventName}' (started by action '${
        actionType
      }'). See details:`,
      {
        eventName,
