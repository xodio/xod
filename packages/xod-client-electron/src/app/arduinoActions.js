...
// :: String -> [Board] -> Board
const findBoardByName = R.curry((boardName, boards) =>
  R.find(R.propEq('name', boardName), boards)
);
  ...
  return R.pipeP(
    ...
    ({ port }) =>
      xad.loadPABs(fqbn, arduinoPackagesPath).then(boards => ({
        port,
        pab: R.compose(R.assoc('cpu', boardCpuId), findBoardByName(boardName))(
          boards
        ),
      })),
    ({ port, pab }) =>
...
export const startDebugSessionHandler = (storeFn, onCloseCb) => (
  ...
  const onClose = () => {
    ...
    onCloseCb(() => {
      ...
      event.sender.send(EVENTS.DEBUG_SESSION, [
        createErrorMessage(closeConnectionErr),
      ]);
    });
    event.sender.send(
