...
describe('IDE', () => {
  ...
  before(() => {
    ...
    return fsp
      .mkdtempP(tmpDir)
      .then(home => {
        process.env.USERDATA_DIR = process.env.HOME = tmpHomeDir = home;
      })
      .then(() => app.start())
      .then(() => {
  ...
  describe('adding nodes to patch', () => {
    ...
    it('adds the rest of the nodes for the blink patch', () =>
      page
        .addNode('digital-output', 150, 300)
  ...
  describe('showing code for arduino', () => {
    ...
    it('shows code', () =>
      app.electron.ipcRenderer
        .emit(TRIGGER_MAIN_MENU_ITEM, ['Deploy', 'Show Code For Arduino'])
        .then(() => page.getCodeboxValue())
        .then(code =>
          assert.strictEqual(
            code,
            expectedCpp,
            'Actual and expected C++ don’t match'
          )
        ));
    it('closes show code popup', () => page.closePopup());
  });

  describe('deleting a patch', () => {
    it('deletes "my-blink" patch', () =>
      page
        .findPatchGroup('welcome-to-xod')
        .click()
